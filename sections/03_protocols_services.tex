...
DAQ CCM requests run-variable and local timing-endpoint configuration through the
...
SC / Ignition & Voltages, temperatures, fans, readout link diagnostics (including SFP I\textsuperscript{2}C monitoring), service status and software/firmware versions. \\
SC and DAQ & Timing-endpoint condition, reset/recovery progress and board readiness. \\
\bottomrule
\end{tabular}
\end{center}

SC and opmon subscribe independently to their respective monitoring data
through the OPC UA bridge. Publications shall carry source identity, time,
sequence and validity. Subscribers shall detect
stale or missing updates and obtain a current snapshot after reconnecting.

\subsubsection{Timing-endpoint control, monitoring and recovery}
\label{sec:timing-recovery}

DTS defines the endpoint controls and status. The endpoint HDL is in
\code{daphne-firmware} [R8]; \code{daphne-os} provides the protocols for accessing
its registers [R4]. The endpoint service reads and controls them locally;
the OPC UA bridge shall expose this access to both DAQ and SC through GbE.

Timing configuration and monitoring use two paths. GbE carries local controls,
including endpoint reset and address configuration, and publishes endpoint
status and timing-SFP I\textsuperscript{2}C status.
The DTS optical fibre carries the remaining timing configuration and
monitoring such as alignment status. Endpoint reset is required before the
DTS configuration sequence.

GbE configuration and control use request/response; monitoring is published
through OPC UA subscriptions.
...
Reset access shall remain available while timing is unavailable. Concurrent
...
endpoint readiness and valid synchronization [R8, R4]. Reset release alone is
insufficient. The interface release shall define how SC receives readiness
evidence carried by DTS. Missing or stale required publications shall withhold readiness.
DAQ checks both board readiness and the detector availability published by SC
before resuming data taking.
...
LCMs use the same timing protocol and split configuration/monitoring paths as
DAPHNE (Section~\ref{sec:timing-recovery}). Both DAQ and SC access local endpoint
configuration and GbE monitoring through the LCM service and OPC UA adapter. The same reset
acknowledgement, command arbitration and access during timing outages apply.
