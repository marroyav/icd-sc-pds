...
DAQ CCM requests run-variable configuration and timing-endpoint reset through the
...
\subsubsection{Timing-endpoint reset, monitoring and recovery}
\label{sec:timing-recovery}

Timing configuration and monitoring use two paths. The GbE CCM link carries
endpoint reset and publishes endpoint status and timing-SFP I\textsuperscript{2}C
status. The DTS optical fibre carries the remaining timing configuration and
monitoring such as alignment status. Endpoint reset is required before the
DTS configuration sequence.

DAQ and SC shall both access endpoint reset and GbE monitoring through the
OPC UA bridge. Reset uses request/response; monitoring is published through
...
DAPHNE (Section~\ref{sec:timing-recovery}). Both DAQ and SC access endpoint reset
and GbE monitoring through the LCM service and OPC UA adapter. The same reset
