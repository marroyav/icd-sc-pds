...
The external OPC UA server integrates DAQ configuration and subscribes to
DAPHNE monitoring. Ignition receives SC publications; the DAQ CCM client
receives published firmware counters for opmon. Hermes retains its separate
configuration and publication paths.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}
\node[box,text width=29mm] (ignition) at (0,0) {Ignition\\SC client};
\node[box,text width=32mm] (daq) at (5.0,0) {DAQ CCM client\\opmon: counters};
\node[box,text width=35mm] (hermesclient) at (10.7,0)
 {DAQ Hermes client\\counter subscriber};
...
\node[box,text width=49mm,minimum height=18mm] (daphne) at (2.5,-5.8)
 {Board-side DAPHNE service\\Configuration endpoint\\Monitoring publisher};
\node[box,text width=37mm,minimum height=18mm] (hermes) at (10.7,-5.8)
 {Board-side Hermes\\IPBus / UDP service\\Counter publisher};
...
\draw[flow,draw=dunegreen] ([xshift=11mm]daphne.north) -- node[right,align=left,font=\footnotesize]
 {Published\\monitoring} ([xshift=11mm]ua.south);
\draw[both] ([xshift=-8mm]hermesclient.south) -- node[left,align=right,font=\footnotesize]
 {IPBus / UDP\\configuration} ([xshift=-8mm]hermes.north);
\draw[flow,draw=dunegreen] ([xshift=8mm]hermes.north) -- node[right,align=left,font=\footnotesize]
 {Published\\counters} ([xshift=8mm]hermesclient.south);
\end{tikzpicture}
\caption{CCM configuration and published monitoring.}
\label{fig:protocol}
\end{figure}

\subsection{Configuration exchanges}

DAQ submits configuration through OPC UA. The server exchanges Protobuf
...
The proposed publication transport is Protobuf over ZeroMQ PUB/SUB over TCP,
using endpoints separate from configuration. The OPC UA server subscribes to
DAPHNE publications and distributes them through OPC UA subscriptions.
Hermes publishes its firmware counters to its DAQ subscriber independently.
...
\midrule
DAQ opmon & Firmware counters published by DAPHNE and Hermes. \\
...
DAQ and SC commands shall be serialized.
...
Each release shall publish online the compatible server/client versions,
Protobuf schemas, OPC UA model, Hermes interface and endpoint/port allocation.
The configuration and publication contracts shall specify authentication,
timeouts, queue limits, update cadence and reconnect behavior. Publication
...
timing recovery with a board-generated completion acknowledgement. The
