...
- Timing configuration and monitoring use two paths: GbE carries endpoint reset, endpoint status and timing-SFP I²C status; DTS fibre carries remaining timing configuration and monitoring such as alignment status. Reset precedes DTS configuration. Both DAQ and SC access reset and GbE monitoring through OPC UA. LCMs follow the same contract through their own service/OPC UA adapter. Reset acknowledgement, arbitration, access during timing loss and readiness remain required; delivery of DTS readiness evidence to SC remains to be specified.
...
- No detailed variable catalogue, data-readout transport or optical timing transport specification is included. The split GbE/DTS configuration and monitoring paths are identified. Seven reference groups remain. Legacy server-repository and superseded source links are absent. EDMS 3309688 is retained as the document ID by author instruction.
...
PDF SHA-256: `5c87da76562c76e712aae8a89a59895231278aa4d1d15278715c9b3d3b0c7077`
